import argparse
from src.Normal_tree import Normal_tree
...
if __name__ == '__main__':
    ...
    normal_tree = Normal_tree(args.num_of_nodes, args.degrees)
    normal_tree_graph = normal_tree.generate_graph(display_graph = str_to_bool[args.display_graph], edges_func = lambda: 1)
    ...
    topo = Topo_info(normal_tree_graph, 24, 3)

    topo.normal_tree_matrix = normal_tree.convert_graph_to_matrix(display_matrix = str_to_bool[args.display_matrix])
    topo.normal_tree_table = normal_tree.convert_graph_to_table(display_table = str_to_bool[args.display_table])

    topo.bandwidth_matrix = generate_matrix_by_input('bandwidth', topo.normal_tree_matrix, topo.node_list, restrict = [10, 15, 20, 25])
    topo.distance_matrix = generate_matrix_by_input('distance', topo.normal_tree_matrix, topo.node_list)
